...
def sigmahh_kukl(ku, kl, channel, confusion):
    coef = confusion[channel]
    return ((coef['ku']*(ku**2)/480.**2 + coef['tri']*kl**2 - coef['int']*kl + coef['box'])*BR_u(ku)/BR_u(1.) + coef['tth+bbh'] + coef['bbxaa'])

def sigmahh_kdkl(kd, kl, channel, confusion):
    coef = confusion[channel]
    return ((coef['kd']*(kd**2)/225.**2 + coef['tri']*kl**2 - coef['int']*kl + coef['box'])*BR_d(kd)/BR_d(1.) + coef['tth+bbh'] + coef['bbxaa'])

def sigmahh_kukd(ku, kd, channel, confusion):
    coef = confusion[channel]
    return ((coef['ku']*(ku**2)/480.**2 + coef['kd']*(kd**2)/225.**2 + coef['hhsm'])*BR_ud(ku, kd)/BR_ud(1., 1.) + coef['tth+bbh'] + coef['bbxaa'])

def sigmahh_kukdkl(ku, kd, kl, channel, confusion):
    coef = confusion[channel]
    return ((coef['ku']*(ku**2)/480.**2 + coef['kd']*(kd**2)/225.**2 + coef['tri']*kl**2 - coef['int']*kl + coef['box'])*BR_ud(ku, kd)/BR_ud(1., 1.) + coef['tth+bbh'] + coef['bbxaa'])

def sigmahh_ku(ku, channel, confusion):
    coef = confusion[channel]
    return ((coef['ku']*(ku**2)/480.**2 + coef['hhsm'])*BR_u(ku)/BR_u(1.) + coef['tth+bbh'] + coef['bbxaa'])

def sigmahh_kd(kd, channel, confusion):
    coef = confusion[channel]
    return ((coef['kd']*(kd**2)/225.**2 + coef['hhsm'])*BR_d(kd)/BR_d(1.) + coef['tth+bbh'] + coef['bbxaa'])